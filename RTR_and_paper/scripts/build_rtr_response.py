...
def build() -> None:
    ...
    add_comment_response(
        ...
        [
            ...
            "The manuscript reports these comparisons in the agreement table and in a per-individual scatter of the 200 pairs. The weights in these data sets have at most six decimal places, and the selected scale represented them without rounding. The exact agreement therefore applies to these data sets and does not establish accuracy for models with more precise weights. For the randomized risk category, all 48 individuals outside the uncertainty range kept the same category. Two individuals were inside the range; one changed category in that run.",
            ...
            "Revised wording: The agreement table gives the sample count and zero MAE/RMSE/maximum error for each model size, the accompanying figure plots every individual against the Equation 1 value, and the surrounding text states 200/200 exact matches and explains why no rounding occurred.",
        ],
    )
