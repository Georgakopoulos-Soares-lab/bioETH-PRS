...
The contracts do not impose a fixed 100-variant model limit; they process larger models in
repeated groups. The four Streaming sizes were evaluated only in the local simulation, and no
new Sepolia Streaming result is reported. The only completed Sepolia calculation remains the
earlier public-weight 100-variant Classic calculation.
